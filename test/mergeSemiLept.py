...
if options.type.lower() == "mutag":
    crabFolders = [name for name in os.listdir("crab_tasks/"+options.date+"/muTag") if os.path.isdir(os.path.join("crab_tasks/"+options.date+"/muTag", name)) and name.startswith("crab_MC_muTag_TTJets_SemiLept")]

    outputName = "crab_results/"+options.date+"/MuTagForRivet_TTJets_SemiLeptMGDecays.root" 
