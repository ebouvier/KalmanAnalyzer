...
for disc in ["pT", "csv"]:
    if options.type.lower() == "d0":
        crabFolders = [name for name in os.listdir("crab_tasks/"+options.date+"/D0/"+disc) if os.path.isdir(os.path.join("crab_tasks/"+options.date+"/D0/"+disc, name)) and (name.startswith("crab_Data_D0_"+disc+"_El") or name.startswith("crab_Data_D0_"+disc+"_SingleEl"))]
    elif options.type.lower() == "mutag":
        crabFolders = [name for name in os.listdir("crab_tasks/"+options.date+"/MuTag/"+disc) if os.path.isdir(os.path.join("crab_tasks/"+options.date+"/MuTag/"+disc, name)) and (name.startswith("crab_Data_MuTag_"+disc+"_El") or name.startswith("crab_Data_MuTag_"+disc+"_SingleEl"))]

    if options.type.lower() == "d0":
        outputName = "crab_results/"+options.date+"/D0ForRivet_"+disc+"_El_merged.root" 
    elif options.type.lower() == "mutag":
        outputName = "crab_results/"+options.date+"/MuTagForRivet_"+disc+"_El_merged.root" 
